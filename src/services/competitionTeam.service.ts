...
export const competitionTeamService = {
  ...
  createTeam: async (data: CreateCompetitionTeamData): Promise<ApiResponse<{ team: CompetitionTeam }>> => {
    const response = await api.post<ApiResponse<{ team: CompetitionTeam }>>('/competition-teams', data);
    return response.data;
  },

  // Get all teams for current student
  getMyTeams: async (): Promise<ApiResponse<{ teams: CompetitionTeam[] }>> => {
    const response = await api.get<ApiResponse<{ teams: CompetitionTeam[] }>>('/competition-teams/my-teams');
    return response.data;
  },

  // Get team by ID
  getTeamById: async (teamId: string): Promise<ApiResponse<{ team: CompetitionTeam }>> => {
    const response = await api.get<ApiResponse<{ team: CompetitionTeam }>>(`/competition-teams/${teamId}`);
    return response.data;
  },

  // Get pending invitations for current student
  getPendingInvitations: async (): Promise<ApiResponse<{ invitations: TeamInvitation[] }>> => {
    const response = await api.get<ApiResponse<{ invitations: TeamInvitation[] }>>('/competition-teams/invitations');
    return response.data;
  },
  ...
  searchStudents: async (query: string): Promise<ApiResponse<{ students: StudentSearchResult[] }>> => {
  ...
  removeMember: async (teamId: string, memberId: string): Promise<ApiResponse<{ team: CompetitionTeam }>> => {
