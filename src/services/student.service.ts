...
export const studentService = {
  ...
  login: async (credentials: {
    email: string;
    password: string;
  }): Promise<ApiResponse<StudentAuthResponse>> => {
    const response = await api.post<ApiResponse<StudentAuthResponse>>(
      "/students/login",
      credentials,
    );
    if (response.data.data) {
  ...
  getAllStudents: async (params?: {
    ...
    const response = await api.get<
      ApiResponse<{ students: Student[]; total: number; page: number; pages: number }>
    >("/students", { params });
    return response.data;
  },
  ...
  getMyEvents: async (): Promise<
    ApiResponse<{ events: Array<{ _id: string; title: string; date: string; status: string }> }>
  > => {
    const response =
      await api.get<
        ApiResponse<{ events: Array<{ _id: string; title: string; date: string; status: string }> }>
